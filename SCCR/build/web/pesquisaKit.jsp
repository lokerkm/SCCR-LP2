...
<html ng-app="testApp">
    ...
    <body ng-controller="testController" ng-init="getRequest()">
        ...
        <nav class="navbar navbar-dark bg-dark navbar-fixed-top">
            <div class="navbar-header">
                ...
                <ol class="breadcrumb" style="margin: auto;">
                    ...
                    <li class="breadcrumb-item active" aria-current="page">Pesquisa de Kit</li>
                </ol>
            </div>
        ...
        <div class="container" style="margin: 20px auto;">
            <input class="form-control"  style="margin-bottom: 7px;" id="buscaNome" ng-model="search.descricao" placeholder="Pesquise por descrições" />
            <table class="table" id="tabelaPesquisa">
                <thead class="thead-dark">
                    <tr>
                        <th scope="co1">Codigo Kit</th>
                        <th scope="co1">Codigo Evento</th>
                        <th scope="co1">Descricao kit</th>
                        <th scope="co1">Produtos</th>
                        <th scope="co1">Camisa</th>
                ...
                <tbody>
                    <tr ng-repeat="kit in kits|filter:search:strict">
                        ...
                        <td>{{kit.descricao}}</td>
                        <td>{{kit.produtos}}</td>
                        <td><span ng-hide="{{kit.camisa}}">Sem </span><span ng-hide="{{!kit.camisa}}">Com </span>Camisa</td>
                        <td ><div style="float: left" <c:if test="${tipoLogin=='atleta'}">hidden</c:if>>
                                <a class="btn btn-warning" href="ManterKitController?acao=prepararEditar&tipoLogin=${tipoLogin}&codKit={{kit.codKit}}&codEvento={{kit.codEvento}}"/>Editar</a>
                            </div>  <div style="float: right" <c:if test="${tipoLogin=='atleta'}">hidden</c:if>>  
                                <a class="btn btn-danger" href="ManterKitController?acao=prepararExcluir&tipoLogin=${tipoLogin}&codKit={{kit.codKit}}&codEvento={{kit.codEvento}}"/>Excluir</a>
                            </div> </td>
                    </tr>
                </tbody>
            </table>
            <form action="ManterKitController?acao=prepararIncluir&tipoLogin=${tipoLogin}" method="post">
        ...
        <script>
                    ...
                    testApp.controller('testController', function ($scope, $http) {
                        $scope.getRequest = function () {
                            $http.get("http://localhost:8080/SCCR/api/kits")
