...
<html>
    ...
    <body>
        <nav class="navbar navbar-dark bg-dark navbar-fixed-top">
            <div class="navbar-header">
                <ol class="breadcrumb" style="margin: auto;">
                    ...
                    <li class="breadcrumb-item"><a href="PesquisaInscricaoController?tipoLogin=${tipoLogin}">Pesquisa de Inscricão</a></li>
        ...
        <div class="container">
            <form action="ManterInscricaoController?acao=confirmar${operacao}&tipoLogin=${tipoLogin}" method="post" name="frmManterInscricao" >
                <div class="card">
                    <div class="card-block">
                        ...
                        <div class="form-row">
                            <div class="form-group col-md-3">
                                ...
                                <select class="form-control" id="evento" name="optEvento" onLoad="confereEvento()" onChange="confereEvento()" <c:if test="${operacao == 'Excluir'}"> readonly</c:if>>
                            ...
                            <div class="form-group col-md-3">
                                ...
                                <select class="form-control" id="percurso" name="optPercurso" onLoad="confereEvento()" onChange="confereEvento()" <c:if test="${operacao == 'Excluir'}" > readonly</c:if>>
                                    <c:forEach items="${percursos}" var="percurso">
                                        <option value="${percurso.codPercurso}|${percurso.codEvento}" <c:if test="${percurso.codPercurso == inscricao.codPercurso} && ${percurso.codEvento == evento.codEvento}"> selected</c:if> >${percurso.nome} </option>
                                    </c:forEach>
                                </select>
                            </div> 
                        </div>
                        ...
                        <div class="form-group row">
                            <div class="col-sm-10">
                                ...
                                <a class ="btn btn-info" href="PesquisaInscricaoController?tipoLogin=${tipoLogin}"> Voltar</a>
                            </div>
                        </div>
                    </div>
                </div>
        ...
        <script>
            function confereEvento() {
                var x = document.getElementById("evento").value;
                var y = document.getElementById("percurso").value.split("|");

                if (y[1] != x) {
                    document.getElementById("btnConfirmar").disabled = true;
                    alert("Você deve escolher um percurso do mesmo evento!")
                } else {
