        ...
        <nav class="navbar navbar-dark bg-dark navbar-fixed-top">
            ...
            <div class="navbar-header">
                <ol class="breadcrumb" style="margin: auto;">
                    ...
                    <li class="breadcrumb-item active" aria-current="page">Pesquisa de Percurso</li>
                </ol>
            </div>
            ...
            <table class="table" id="tabelaPesquisa">
                ...
                <tbody>
                    <tr ng-repeat="percurso in percursos| filter:search:strict">
                        <td>{{percurso.codPercurso}}</td>
                        <td>{{percurso.codEvento}}</td>
                        <td>{{percurso.nome}}</td>
                        <td ><div style="float: left" <c:if test="${tipoLogin=='atleta'}">hidden</c:if>>
                                <a class="btn btn-warning" <c:if test="${tipoLogin!='atleta'}"> href="ManterPercursoController?acao=prepararEditar&tipoLogin=${tipoLogin}&codPercurso={{percurso.codPercurso}}&codEvento={{percurso.codEvento}}"</c:if>/>Editar</a>
                            </div>  <div style="float: center" <c:if test="${tipoLogin!='administrador'}">hidden</c:if>>  
                                <a class="btn btn-danger" <c:if test="${tipoLogin!='atleta'}"> href="ManterPercursoController?acao=prepararExcluir&tipoLogin=${tipoLogin}&codPercurso={{percurso.codPercurso}}&codEvento={{percurso.codEvento}}"</c:if>/>Excluir</a>
                                </div>                            
                            </td>
                        </tr>
                    </tbody>
                </table>
                <form <c:if test="${tipoLogin!='atleta'}">action="ManterPercursoController?acao=prepararIncluir&tipoLogin=${tipoLogin}" </c:if>method="post">
        ...
        <script>
                    ...
                    testApp.controller('testController', function ($scope, $http) {
                        $scope.getRequest = function () {
                            $http.get("http://localhost:8080/SCCR/api/percursos")
