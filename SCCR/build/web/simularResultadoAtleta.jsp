...
<html>
    <head>
        ...
        <title>Simular Resultado Atleta</title>
    </head>
    <style>
        html{
            position: relative;
    ...
    <body>
        <nav class="navbar navbar-dark bg-dark navbar-fixed-top">
            <div class="navbar-header">
                <ol class="breadcrumb" style="margin: auto;">
                    ...
                    <li class="breadcrumb-item active" aria-current="page">Simular Resultado de Atleta</li>
                </ol>
            </div>
        ...
        <div class="container" >
            <form action="ManterAtletaController?acao=confirmaSimularResultado&tipoLogin=${tipoLogin}" method="post" name="frmSimularResultadoAtleta">
                <div class="card">
                    <div class="card-block">
                            ...
                            <div class="form-group col-md-3">
                                ...
                                <select class="form-control" id="evento" name="optEvento" <c:if test="${operacao == 'Excluir'}"> readonly</c:if>>
                                    <c:forEach items="${eventos}" var="evento">
                                        <option value="${evento.codEvento}" <c:if test="${kit.codEvento== evento.codEvento}"> selected</c:if>>${evento.titulo}</option>  
                                    </c:forEach>
                                </select>
                            </div>
                        </div>
                            ...
                            <div class="form-group col-md-3">
                                ...
                                <select class="form-control" id="atleta" name="optAtleta" <c:if test="${operacao == 'Excluir'}"> readonly</c:if>>
                                    <c:forEach items="${atletas}" var="atleta">
                                        <option value="${atleta.codAtleta}" >${atleta.nome}</option>  
                                    </c:forEach>
                                </select>
                            </div>
                            <div class="form-group col-md-2">
                                ...
                                <select class="form-control" id="chip"name="optChip" <c:if test="${operacao == 'Excluir'}"> readonly</c:if>>
                                    <c:forEach items="${chips}" var="chip">
                                        <option value="${chip.codChip}">${chip.codChip}</option>  
                                    </c:forEach>
                                </select>
                            </div>
                        </div>
                        ...
                        <div class="form-row">
                            <div class="form-group col-md-3">
                                ...
                                <input class="form-control" id="codPagamento" type="text" name="txtTempo" value="${atleta.distancia}" >
                            </div>
                        </div>
                        <div class="form-row">
                            <div class="form-group col-md-3">
                                <label for="tempo">Codigo Pontuacao</label>
                                <input class="form-control" id="posicao" type="text" name="txtCodPontuacao"  >
                            </div>
                             <div class="form-group col-md-3">
                                <label for="tempo">Posição</label>
                                <input class="form-control" id="posicao" type="text" name="txtPosicao"  >
                            </div>
                        </div>
                    </div>
